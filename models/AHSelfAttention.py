...
class AHSelfAttention(nn.Module): 
    def __init__(self, dim, heads=8, dim_head=None, causal=True):  # e.g., dim=512 (embedding dim) 
        super().__init__() 
    ...
    def forward(self, x, mask=None): 
        ...
        # Decompose to q, k, v and reshape to [3, b, heads, seq length, dim_head] 
        q, k, v = tuple(rearrange(qkv, 'b n (d k h) -> k b h n d', k=3, h=self.heads)) 

        # Calculate scaled dot product attention: [batch, heads, tokens, tokens]
        scaled_dot_prod = torch.einsum('b h i d, b h j d -> b h i j', q, k) * self.scale_factor 

        # Mask for causality during training 
